 ...
 * (<code>&lt;br&gt;</code>, <code>&lt;p&gt;</code>, <code>&lt;b&gt;</code>,
 * <code>&lt;i&gt;</code>,
 ...
 * characters, and Form Feed characters (U+000C)
 * are converted to "<code>&lt;br /&gt;</code>". CR/LF pairs are treated as a
 * single line break.
...
public class HTMLSanitiser {
	...
	private static final Set<String> VALID_ELEMENT_NAMES = new HashSet<String>(
		Arrays.asList( new String[] { HTMLElementName.BR, HTMLElementName.P, HTMLElementName.B, HTMLElementName.I,
	...
	private static final Set<String> VALID_ATTRIBUTE_NAMES = new HashSet<String>(
		Arrays.asList( new String[] { "id", "class", "href", "target", "title" } ) );
	 ...
	 * <code>formatWhiteSpace</code> parameter
	 * results in the formatting of white space as described in the sanitisation
	 * process in the class description above.
	 * <p>
	 * <dl>
	 * <dt><b>Example:</b></dt>
	 * <dd>
	...
	private static String sanitise( String pseudoHTML, boolean formatWhiteSpace, boolean stripInvalidElements ) {
		...
		for( Tag tag : tags ) {
			if( tag.getBegin() < pos )
				continue; // this might happen if a script element was encountered
			if( processTag( tag, outputDocument ) ) {
				...
				if( tag.getName() == HTMLElementName.SCRIPT && tag.getTagType() == StartTagType.NORMAL ) {
					...
					if( endTag == null ) {
						// script has no end tag, remove everything to the end of the source and don't process any more tags
	...
	public static void main( String[] args ) throws Exception {
		...
		displayEncodeInvalidMarkup( "<p class=\"xyz\" onmouseover=\"nastyscript\">abc</p>",
		...
		displayStripInvalidMarkup( "abc<script>def</script>geh", "remove potentially dangerous script" );
		displayStripInvalidMarkup( "<p class=\"xyz\" onmouseover=\"nastyscript\">abc</p>",
			"keep approved attributes but strip non-approved attributes" );
